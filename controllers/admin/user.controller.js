...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (req.query.isChannel === "true") {
      ...
      return res
        .status(200)
        .json({ status: true, message: "finally, update details of the channel or profile of the user by admin!", user: data });
  ...
  try {
    ...
    await Promise.all(
      users.map(async (user) => {
        //delete userImage from DigitalOcean Spaces
        if (user?.image) {
          const urlParts = user?.image.split("/");
    ...
    if (result.deletedCount > 0) {
      return res.status(200).json({ status: true, message: "finally, user has been deleted by admin!" });
    }
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, error: error.message || "Internal Server Error" });
...
exports.getProfile = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, profile of the user get by admin.", user: data });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
exports.channelsOfUser = async (req, res) => {
  try {
    ...
    if (req.query.type === "addByadmin") {
      ...
      return res.status(200).json({
        status: true,
        message: "finally, get the all channels of the user who has been added by admin!",
      ...
      return res.status(200).json({
        status: true,
        message: "finally, get the all channels of the real user!",
        totalChannels: totalChannels,
        channels: channels,
      });
    } else {
