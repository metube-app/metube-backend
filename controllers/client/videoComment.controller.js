...
exports.createComment = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, comment passed on video by that user!", videoComment });
  } catch (error) {
    console.log(error);
...
exports.likeOrDislike = async (req, res) => {
  try {
    ...
    const [user, videoComment] = await Promise.all([
      User.findOne({ _id: req.query.userId, isActive: true }),
      VideoComment.findById(req.query.videoCommentId),
    ]);
...
exports.createCommentReply = async (req, res) => {
  try {
    ...
    const [user, video, videoCommentData] = await Promise.all([
      User.findOne({ _id: req.body.userId, isActive: true }),
      Video.findOne({ _id: req.body.videoId, isActive: true }),
    ...
    videoComment.videoId = video._id;
    videoComment.commentText = req.body.commentText;
    videoComment.recursiveCommentId = videoCommentData._id;
    await videoComment.save();

    videoCommentData.totalReplies += 1;
    await videoCommentData.save();

    return res.status(200).json({ status: true, message: "finally, comment reply given by user for that video!", videoComment });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, message: error.message || "Internal Server Error" });
  }
